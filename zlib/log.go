...
func  (log *Log)OpenNewFd( )(err error ){
	...
	fd, err  := os.OpenFile(pathFile, os.O_WRONLY | os.O_CREATE | os.O_APPEND , 0666)
	if err != nil{
...
func  (log *Log)  checkOutFilePathPower(path string)error{
	...
	if perm < os.FileMode(0600).String(){
		return errors.New(" checkOutFilePathPower ("+path+"):path permission 0600 ")
	}
	return nil
}
