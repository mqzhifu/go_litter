...
func  (gamematch *Gamematch)closeOneRuleDemonRoutine( ruleId int )int{
	...
	mylog.Info("closeOneRuleDemonRoutine ,ruleId :  ",ruleId , " finish." )
...
func (gamematch *Gamematch)signSend(mychan chan int,data int,keyword string){
	mylog.Warning("SIGNAL send blocking.... ",keyword)
	mychan <- data
...
//开启一个守护协程，这里只是统一管理
//handler:每秒回调一次这个函数
func (gamematch *Gamematch)StartOneGoroutineDemon( ruleId int ,title string,demonLog *zlib.Log, handler func( )){
