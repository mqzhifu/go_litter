...
func (queueSign *QueueSign) cancelByGroupId(  groupId int ) error {
	...
	queueSign.Log.Info("cancelByGroupId groupId : ",groupId)
	//mylog.Debug(group)
	...
	if group.SignTime > zlib.GetNowTimeSecondToInt(){
		mylog.Warning("group timeout:",groupId)
		queueSign.Log.Warning("group timeout:",groupId)
	}
	//检查每个玩家的报名时间 ，是否已经超时了
...
func (queueSign *QueueSign) getGroupElementById(  id int ) (group  Group){
	...
	if res == ""{
		mylog.Error(" getGroupElementById is empty," ,id)
		return group
	}
...
func (queueSign *QueueSign) getGroupPersonIndexList( personNum int, rangeStart string,rangeEnd string,limitOffset int ,limitCnt int,isDel bool)(ids map[int]int){
	...
	if isDel{
		redisConn := myredis.GetNewConnFromPool()
		defer redisConn.Close()
		mylog.Info("getGroupPersonIndexList del group:",res)
...
func  (queueSign *QueueSign) CheckTimeout( ){
	...
	if len(res) == 0{
		//每调用10次输出一行日志，不然日志太多
		if now % 10 == 0{
			queueSign.Log.Notice("queueSign CheckTimeout empty , no need process")
		}
